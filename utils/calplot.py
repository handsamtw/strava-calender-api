...
def yearplot(
    ...
    **kwargs
    ...
    by_day = by_day.reindex(
        pd.date_range(
            start=str(year), end=str(year + 1), freq="D", tz=by_day.index.tzinfo
        )[:-1]
    )
    ...
    ax.set_xticks(
        [
            by_day.loc[pd.Timestamp(datetime.date(year, i + 1, monthlabeloffset))].week
            for i in monthticks
        ]
    )
    ...
    if textformat is not None:
        for y in range(plot_data.shape[0]):
            for x in range(plot_data.shape[1]):
                ...
                ax.text(
                    x + 0.5, y + 0.5, content, color=textcolor, ha="center", va="center"
                )
...
def calplot(
    ...
    **kwargs
    ...
    ylabel_kws = dict(
        fontsize=30, color="gray", fontname="Helvetica", fontweight="bold", ha="center"
    )
    ...
    for year, ax in zip(years, axes):
        ...
        if ax_title:
            ...
            ax.text(s=textstr, transform=ax.transAxes, **ax_text_attributes)
